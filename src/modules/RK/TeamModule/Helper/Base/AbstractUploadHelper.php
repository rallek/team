...
abstract class AbstractUploadHelper
{
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('RKTeamModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('RKTeamModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('RKTeamModule', 'shrinkHeight' . $fieldSuffix, 600);
                ...
                if ($imgInfo[0] > $maxWidth || $imgInfo[1] > $maxHeight) {
                    ...
                    $preset = new \SystemPlugin_Imagine_Preset('RKTeamModule_Shrinker', [
                        'width' => $maxWidth,
                        'height' => $maxHeight,
                        'mode' => 'inset'
                    ]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/RKTeamModule/';
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        $htaccessFileTemplate = 'modules/RK/TeamModule/Resources/docs/htaccessTemplate';
