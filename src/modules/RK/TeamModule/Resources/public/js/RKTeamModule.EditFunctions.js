 ...
 */
function rKTeamInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('rkteammodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function rKTeamResetUploadField(fieldName)
{
 ...
 */
function rKTeamInitUploadField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        rKTeamResetUploadField(fieldName);
    }).removeClass('hidden');
}
...
function rKTeamTriggerFormValidation()
{
    rKTeamExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function rKTeamHandleFormSubmit (event) {
    if (triggerValidation) {
        rKTeamTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function rKTeamInitEditForm(mode, entityId)
{
    if (jQuery('.rkteam-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.rkteam-edit-form').first();
    ...
    editForm.submit(rKTeamHandleFormSubmit);
